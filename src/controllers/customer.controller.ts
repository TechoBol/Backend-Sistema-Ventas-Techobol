...
export const editCustomer = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    const { name, businessName, code, phone, whatsapp, occupation, originChannel } = req.body;
    ...
    if (error.code === "P2002") {
      return res.status(409).json({ message: "Código ya registrado" });
    }
...
export const storeNit = async (req: Request, res: Response) => {
  try {
    const customerId = Number(req.params.id);
    const { number, companyName } = req.body;

    if (!number) {
      return res.status(400).json({ message: "El número de NIT es requerido" });
    }

    // Solo dígitos
    if (!/^\d+$/.test(number)) {
      return res.status(400).json({ message: "El NIT solo debe contener números" });
    }

    const nit = await addCustomerNit(customerId, { number, companyName });
    return res.status(201).json(nit);
  } catch (error: any) {
    if (error.code === "P2002") {
      return res.status(409).json({ message: "Este NIT ya está registrado para este cliente" });
    }
    return res.status(500).json({ message: "Error al agregar NIT", error });
  }
};
